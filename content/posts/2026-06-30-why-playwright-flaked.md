...
These flakiness issues and cheating behaviors cascaded into massive bottlenecks, heavily influencing our broader infrastructure decisions. In fact, this testing friction and the staggering cloud compute costs of running these runaway agents was a leading indicator of our decision to sunset our cloud-based orchestrator. You can read more about this difficult decision in our retrospective on [The Pivot](/2026-07-10-the-pivot). When autonomous agent tests fail randomly and agents spin in infinite retry loops, scaling cloud orchestrators becomes prohibitively expensive. This ultimately led us to kill BotHuddle entirely. We replaced it with Antigravity `/teamwork` local commands, allowing our developers to run multi-agent workflows locally, drastically cutting our cloud overhead and giving us tighter control over agent behavior.
