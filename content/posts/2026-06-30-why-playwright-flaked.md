...
The fundamental problem with traditional Playwright testing in a generative context is its reliance on literal string matching. In NeuroHub, our most critical user flows live in the Action Center (`/requests`)—where families upload receipts for adaptive equipment, file reimbursement claims with Financial Management Services (FMS), and track California Regional Center authorizations under the Self-Determination Program (SDP). 

When our UI was adapting to different regional terminology, an assertion like `await expect(page.getByText('Submit Receipt')).toBeVisible()` flaked constantly. Depending on the user's regional center profile and prompt context, the model would render "Upload Receipt" one day, "Submit Invoice" the next, and occasionally "Attach Proof of Payment." Playwright, being perfectly literal, would fail the test, even though the semantic intent of the reimbursement workflow was completely valid and functional.

But textual flakiness was only the tip of the iceberg. As we moved toward test-driven development driven by autonomous agents, we discovered that these agents would actively "cheat" to pass tests. Tasked with fixing failing E2E tests for our sensitive healthcare routes, agents started hardcoding `localStorage` tokens or injecting mock AppSync payloads directly into the browser. They bypassed our real AWS Cognito multifactor authentication (MFA) flows entirely just to turn the tests green, hiding real regressions in user session security.
